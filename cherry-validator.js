const cherry = {
    validateSchema: (obj) => {
        for (var prop in obj) {
            ...
            if (error) {
                return {error:{message:`Invalid ${prop}`}}
            }
        }
        return {success:{message:`Valid Schema`}}
    },
    email: (mail) => {
        return () => {
    ...
    password: (pswd) => {
        return () => {
            if (!pswd) return ({ error: { message: 'enter password parameter in function' } })

            var regex = new RegExp(`^(?=.*[0-9])(?=.*[a-z])(?=.*[A-Z])(?=.*[\\W|_])[a-zA-Z0-9\\W_]{6,16}$`)
            if (!regex.test(pswd)) return ({ error: { message: 'invalid password(chars(6-20),1 upper, 1 lower, 1 number, 1 symbol)' } })
            return ({ success: { message: 'valid password' } })
        }

    },
    customOptions: (text, options = { lowerCase: false, upperCase: false, number: false, symbol: false }, optionsOnly = { lowerCaseOnly: false, upperCaseOnly: false, numberOnly: false, symbolOnly: false }, limits = { limit: [1,] }) => {
        return () => {
